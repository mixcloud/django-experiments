...
class WebUserTests(object):
    ...
    def test_enrollment_initially_control(self):
        ...
        self.assertEqual(experiment_user.get_alternative(
            EXPERIMENT_NAME), 'control', "Default Enrollment wasn't control")

    def test_user_enrolls(self):
        experiment_user = participant(self.request)
        experiment_user.set_alternative(EXPERIMENT_NAME, TEST_ALTERNATIVE)
        self.assertEqual(experiment_user.get_alternative(
            EXPERIMENT_NAME), TEST_ALTERNATIVE, "Wrong Alternative Set")
    ...
    def test_record_goal_increments_counts(self):
        ...
        self.assertEqual(self.experiment_counter.goal_count(
            self.experiment, TEST_ALTERNATIVE, TEST_GOAL), 0)
        experiment_user.goal(TEST_GOAL)
        self.assertEqual(self.experiment_counter.goal_count(
            self.experiment, TEST_ALTERNATIVE, TEST_GOAL), 1,
    ...
    def test_can_record_goal_multiple_times(self):
        ...
        self.assertEqual(self.experiment_counter.goal_distribution(
            self.experiment, TEST_ALTERNATIVE, TEST_GOAL), {3: 1},
    ...
    def test_counts_increment_immediately_once_confirmed_human(self):
        ...
        self.assertEqual(self.experiment_counter.participant_count(
            self.experiment, TEST_ALTERNATIVE), 1,
    ...
    def test_user_force_enrolls(self):
        ...
        experiment_user.enroll(
            EXPERIMENT_NAME, ['control', 'alternative1', 'alternative2'],
    ...
    def test_user_does_not_force_enroll_to_new_alternative(self):
        ...
        self.assertEqual(alternative, experiment_user.get_alternative(
            EXPERIMENT_NAME))
...
class WebUserAnonymousTestCase(WebUserTests, TestCase):
    ...
    def test_confirm_human_increments_participant_count(self):
        ...
        self.assertEqual(
                self.experiment_counter.participant_count(
                    self.experiment, TEST_ALTERNATIVE), 0,
...
class BotTests(object):
    def setUp(self):
        self.experiment = Experiment(
            name='backgroundcolor', state=ENABLED_STATE)
        self.experiment.save()
        self.experiment_counter = ExperimentCounter()
    ...
    def test_bot_in_control_group(self):
        self.experiment_user.set_alternative(EXPERIMENT_NAME, TEST_ALTERNATIVE)
        self.assertEqual(
            self.experiment_user.get_alternative(
                EXPERIMENT_NAME), 'control', "Bot enrolled in a group")
        self.assertEqual(
            self.experiment_user.is_enrolled(
                self.experiment.name, TEST_ALTERNATIVE), False,
            "Bot in test alternative")
        self.assertEqual(
            self.experiment_user.is_enrolled(
                self.experiment.name, CONTROL_GROUP), True,
...
class ParticipantCacheTestCase(TestCase):
    def setUp(self):
        self.experiment = Experiment.objects.create(
            name='test_experiment1', state=ENABLED_STATE)
        self.experiment_counter = ExperimentCounter()
...
class ConfirmHumanTestCase(TestCase):
    def setUp(self):
        ...
        self.alternative = self.experiment_user.enroll(
    ...
    def test_confirm_human_updates_experiment(self):
        ...
        self.assertEqual(
            self.experiment_counter.participant_count(
                self.experiment, self.alternative), 0)
        self.assertEqual(
            self.experiment_counter.goal_count(
                self.experiment, self.alternative, 'my_goal'), 0)
        self.experiment_user.confirm_human()
        self.assertNotIn('experiments_goals', self.experiment_user.session)
        self.assertEqual(
            self.experiment_counter.participant_count(
                self.experiment, self.alternative), 1)
        self.assertEqual(
            self.experiment_counter.goal_count(
                self.experiment, self.alternative, 'my_goal'), 1)
    ...
    def test_confirm_human_sets_session(self):
        self.assertFalse(
            self.experiment_user.session.get(
                conf.CONFIRM_HUMAN_SESSION_KEY, False))
        self.experiment_user.confirm_human()
        self.assertTrue(
            self.experiment_user.session.get(
                conf.CONFIRM_HUMAN_SESSION_KEY, False))
...
class DefaultAlternativeTestCase(TestCase):
    def test_default_alternative(self):
        ...
        self.assertEqual(
            conf.CONTROL_GROUP, participant(session=DatabaseSession()).enroll(
                'test_default', ['alt1', 'alt2']))
        experiment.set_default_alternative('alt2')
        experiment.save()
        self.assertEqual(
            'alt2', participant(session=DatabaseSession()).enroll(
                'test_default', ['alt1', 'alt2']))
        experiment.set_default_alternative('alt1')
        experiment.save()
        self.assertEqual(
            'alt1', participant(session=DatabaseSession()).enroll(
                'test_default', ['alt1', 'alt2']))
