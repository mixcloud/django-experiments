...
class ExperimentCounter(object):
    ...
    def participant_count(self, experiment, alternative):
        return counters.get(PARTICIPANT_KEY % (experiment.name, alternative))

    def goal_count(self, experiment, alternative, goal):
        return counters.get(GOAL_KEY % (experiment.name, alternative, goal))

    def participant_goal_frequencies(self, experiment, alternative, participant_identifier):
        for goal in conf.ALL_GOALS:
            yield goal, counters.get_frequency(GOAL_KEY % (experiment.name, alternative, goal), participant_identifier)

    def goal_distribution(self, experiment, alternative, goal):
        return counters.get_frequencies(GOAL_KEY % (experiment.name, alternative, goal))

    def delete(self, experiment):
        counters.reset_pattern(experiment.name + "*")
