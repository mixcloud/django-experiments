...
def experiment_goal(goal_name):
    return {
        'url': reverse(
            'experiment_goal', kwargs={
                'goal_name': goal_name, 'cache_buster': uuid4()})}
...
def experiments_confirm_human(context):
    ...
    return {
        'confirmed_human': request.session.get(
...
def experiment(parser, token):
    ...
    If the alternative name is neither 'test' nor 'control' an exception is
